...
def get_args():
    ...
    parser.add_argument('--num_epoch',type=int,default=3,help='train epochs')
    parser.add_argument('--batch_size', type=int, default=32, help='batch_size')
    parser.add_argument('--log_steps', type=int, default=300, help='steps for logging')
    parser.add_argument('--model_save_path', type=str, default='checkpoints/cifar10_lr.pth', help='save path for model')
    parser.add_argument('--data_path', type=str, default='data/cifar10', help='save path for model')
    parser.add_argument('--predict_dir', type=str, default='data/cifar10/test_dir', help='save path for model')
...
def gen_dataloader(data_path,batch_size=32,size=28):
    ...
    test_dataset = datasets.CIFAR10(root=data_path, train=False, download=True, transform=transform)  # train=True训练集，=False测试集
...
def main():
    ...
    if not args.do_predict:
        train_dataloader,test_dataloader=gen_dataloader(args.data_path,batch_size=args.batch_size,size=224)
        #model=LR(class_num=args.class_num,chanels=3,size=224)
        #model=Lenet5(class_num=args.class_num,chanels=3)
        #model=AlexNet(class_num=args.class_num,chanels=1,size=28)
        #model=GoogLeNet(class_num=args.class_num,chanels=1)
        #model=resnet18(pretrained=False,class_num=args.class_num,chanels=1)
